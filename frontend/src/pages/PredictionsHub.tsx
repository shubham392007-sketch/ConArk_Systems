      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        {modelsData.map(m => (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <h2 style={{ fontFamily: 'Anton, sans-serif', fontSize: '36px', color: '#111111', textTransform: 'uppercase' }}>
                    {m.title}
                  </h2>
                </div>
                <div style={{ fontSize: '13px', fontFamily: 'JetBrains Mono, monospace', color: '#444444', marginTop: '4px' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '52px', fontWeight: '800', color: '#111111', lineHeight: '1' }}>
                  {m.output}
                </div>
                {m.badge && (
                  <span style={{ display: 'inline-block', fontSize: '12px', fontFamily: 'JetBrains Mono, monospace', fontWeight: '800', backgroundColor: '#111111', color: '#FFFFFF', padding: '3px 10px', borderRadius: '4px', marginTop: '6px' }}>
                    {m.badge}
                  </span>
                )}
                {m.confidence && (
                  <div style={{ fontSize: '13px', fontFamily: 'JetBrains Mono, monospace', fontWeight: 'bold', marginTop: '6px' }}>
            ...
            {m.riskFactors && (
              <div style={{ marginTop: '20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                {m.riskFactors.map(rf => (
                  <div key={rf.name} style={{ backgroundColor: 'rgba(255,255,255,0.7)', padding: '10px 14px', borderRadius: '8px', border: '1.5px dashed #111111' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', fontFamily: 'JetBrains Mono, monospace', fontWeight: 'bold' }}>
                      <span>{rf.name}</span>
                      <span>{rf.pct}%</span>
                    </div>
            ...
            {m.whyReason && (
              <div style={{ marginTop: '16px', padding: '14px 18px', backgroundColor: '#FFFFFF', borderRadius: '8px', border: '1.5px dashed #111111', fontSize: '14px', fontFamily: 'Inter, sans-serif' }}>
                <strong>WHY:</strong> "{m.whyReason}"
              </div>
            )}

            {/* Traceability Footer & SHAP Drawer Trigger */}
            <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px dashed rgba(17,17,17,0.25)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px', fontFamily: 'JetBrains Mono, monospace' }}>
