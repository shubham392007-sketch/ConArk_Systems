"""
Train Time / Schedule Forecasting regression model.
...
logger = get_logger("train_time")


def train_time_model(train_df: pd.DataFrame, val_df: pd.DataFrame) -> Tuple[Any, Dict[str, Any]]:
    """Trains and selects best Time Forecasting model."""
    feature_cols = [c for c in ENGINEERED_FEATURE_NAMES if c in train_df.columns]
    
    y_train = train_df["time_deviation"]
    y_val = val_df["time_deviation"]
    ...
    for name, model in candidates.items():
        logger.info(f"Training Time candidate: {name}")
    ...
    logger.info(f"Selected best Time model: {best_name} with RMSE = {best_rmse:.4f}")

    metadata = {
        "model_name": "conark_time_model",
        "version": "1.0.0",
        "algorithm": best_name,
    ...
    registry.save_model("time", best_model, metadata)
...
if __name__ == "__main__":
    ...
    train_time_model(train_df, val_df)
