...
class AnalyticsService:
    ...
    def get_user_analytics(user_id: str) -> Dict[str, Any]:
        ...
        with get_db_cursor() as cur:
            # 1. Total counts
            cur.execute("SELECT COUNT(*) as total FROM public.projects WHERE user_id = %s", (user_id,))
            total_projects = cur.fetchone()["total"]

            cur.execute("SELECT COUNT(*) as total FROM public.model_predictions WHERE user_id = %s", (user_id,))
            total_predictions = cur.fetchone()["total"]

            cur.execute("SELECT COUNT(*) as total FROM public.optimization_results WHERE user_id = %s", (user_id,))
            total_optimizations = cur.fetchone()["total"]

            cur.execute("SELECT COUNT(*) as total FROM public.ai_conversations WHERE user_id = %s", (user_id,))
            total_conversations = cur.fetchone()["total"]

            cur.execute("SELECT COUNT(*) as total FROM public.saved_reports WHERE user_id = %s", (user_id,))
            ...
            cur.execute(
                ...
                FROM public.model_predictions
                WHERE user_id = %s AND model_name = 'risk_intelligence'
